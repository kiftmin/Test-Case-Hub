# Test Case Hub — Current Application State Prompt

## Overview

This document is the single source of truth for the **current state** of Test Case Hub, a full-stack UAT Test Case Management System. It incorporates all changes applied since the original build prompt. Build/continue development based on this specification. Do not make assumptions; follow this document precisely.
...
- **Auth**: JWT stored in `localStorage`, sent as `Bearer` token in `Authorization` header. The API client must call `setAuthTokenGetter(getAuthToken)` at app startup in `main.tsx`. All raw `fetch()` calls must include the Bearer token from `getAuthToken()`.
...
is_active (boolean, default true, not null), created_at
```
- `is_active` controls account suspension. Suspended users cannot log in (403 response).
...
created_at
```
- `passed_by_agreement` is a valid status value.
...
accepted_by_business_note (text, nullable),
rejection_log (text/JSON, nullable),
created_at, updated_at
```
...
```
- `meeting_type` uses snake_case values: `defect_review`, `post_mortem`. Frontend formats for display as "Defect Review", "Post-Mortem".
...
### Auth
- `POST /api/auth/login` — `{ username, password }` → `{ token, user }`. Returns 403 if account is suspended.
- `POST /api/auth/register` — Admin only — `{ username, password, name, email, role }` — role must be `ADMIN` or `USER` (not `AUTHOR` or `TESTER`).

### Users
- `GET /api/users` — Admin only — list all users
- `POST /api/users` — Admin only — create user
- `PUT /api/users/:userId` — Admin only — update user (name, email, role)
- `PUT /api/users/:userId/suspend` — Admin only — toggle `is_active` (suspend/unsuspend)
- `PUT /api/users/:userId/password` — Admin only — change password `{ password }`
- `DELETE /api/users/:userId` — Admin only — returns 403 if user is last remaining admin; returns 409 if user has project assignments (prevents orphaned data)
...
- `POST /api/projects/:projectId/sign-off` — Test Lead or Business Owner — dual signature sign-off
- `GET /api/projects/:projectId/sign-off-status` — check sign-off status for both parties
...
- `DELETE /api/projects/:projectId/users/:userId` — Admin or project TEST_LEAD — Test Lead cannot self-remove (returns 400)
...
- `DELETE /api/test-runs/:testRunId` — Admin or project TEST_LEAD — delete a scheduled (not in_progress/completed) test run
...
- `GET /api/dashboard/tester/:userId/test-runs` — returns runs with status filter including `completed`
- `POST /api/test-runs/:testRunId/test-cases/:testCaseId/execute` — create execution. Returns 403 if test run status is `"completed"`. Returns 403 if this test case already has an execution in this test run (a test case cannot be executed twice in the same run).
- `PATCH /api/executions/:executionId` — update status, result, notes. Auto-creates defect on failure.
...
### Team Discussions
- `POST /api/test-runs/:testRunId/discussions` — Test Lead — `{ meetingType, participantIds }` — `meetingType` must be `"defect_review"` or `"post_mortem"`
...
### Test Run Deletion
A scheduled test run can be deleted (Test Lead or Admin only). Runs with status `in_progress` or `completed` cannot be deleted.

### Execution Guards
- New executions cannot be created for a test run with status `"completed"` (returns 403).
- A test case cannot have more than one execution in the same test run. Creating a second execution for the same test case in the same test run returns 403.
- When a test case already has a completed execution in the current test run, the frontend shows read-only results (step-by-step pass/fail with actual results and comments) instead of "Start New Execution".

### Execution Scoping
All execution data (sidebar indicators, active execution, last completed execution) is scoped by `testRunId`. Executions from other test runs do not appear as checkmarks/X marks or interfere with execution state. The `key` prop on TestCaseExecutor forces a React remount when `testRunId` changes, and query cache is invalidated on mount to avoid stale data.

### Tester Dashboard
- Shows assigned Test Runs for the logged-in tester. Each card shows: project name, run name, scheduled date, countdown timer, number of assigned use cases, pending count.
- Badges indicate run status based on the ratio of `myPendingCount` vs `myUseCaseCount`:
  - `myPendingCount === myUseCaseCount` → **NEW** (blue)
  - `myPendingCount > 0 && myPendingCount < myUseCaseCount` → **IN PROGRESS** (orange)
  - `myPendingCount === 0` → **COMPLETED** (green)
- "Start Testing" button for NEW runs, "View" button for COMPLETED runs, "Locked" for unavailable runs.
- The "Start" button navigates to the test execution view; the "View" button opens the run in read-only mode.
- The Test Execution View shows a full-page "Test Run Submitted" message when `testRun.status === "completed"`.
...
- **Meeting Type**: `defect_review` or `post_mortem` (snake_case values; frontend maps to "Defect Review" / "Post-Mortem" for display)
...
### Account Suspension
- Users can be suspended (Admin only) via `PUT /api/users/:userId/suspend`.
- Suspended users cannot log in (returns 403 "Account has been suspended").
- On the Project Users page, suspended users show a "Suspended" badge, dimmed card, and "(Suspended)" label in role dropdowns.

### Password Management
- Admin can change any user's password via `PUT /api/users/:userId/password`.
- Password change is available in the Edit User dialog.

### User Deletion Safeguards
- The last remaining system admin cannot be deleted (frontend hides delete button; backend returns 403).
- Users with project assignments cannot be deleted (backend returns 409 — prevents orphaned records).

### Test Lead Protection
- A Test Lead cannot remove themselves from a project's user list (backend returns 400; frontend shows "Cannot remove yourself" tooltip).
...
When creating a user, the Admin selects a global role: `ADMIN` or `USER`. The `USER` role is displayed as "User" in the UI. The Users page shows all users with their global role, displayed with distinct colour badges: purple for ADMIN, blue for USER.

### Login Page
Single login for all roles. After login, redirect to Dashboard. No separate "Tester Login" that only uses a project code — all users log in via username/password. The tester project-code entry is for accessing the execution interface once logged in. Suspended users see an error message on login.

### User Management Page (`/users`)
- Admin only. Lists all users with global role badges.
- Create User: username, name, email, password, role (ADMIN or USER).
- Edit User dialog: update name, email, role; suspend/unsuspend toggle; password change field with confirmation.
- Delete: confirms before deleting. Admin users cannot delete their own account if they are the last admin. Users with project assignments get a 409 error.
...
### Project Users Page (`/projects/:id/users`)
- Lists all users assigned to the project with their roles.
- Add user: dropdown with role selection.
- Remove user: confirmation dialog. Test Lead cannot remove themselves (tooltip: "Cannot remove yourself").
- Suspended users display a "Suspended" badge, dimmed card background, and "(Suspended)" label in role dropdowns.

### Test Run List
- Shows all test runs for a project with status badges and scheduled dates.
- Create test run: name, scheduled date/time, optional use case selection.
- Delete scheduled test runs: Delete button with confirmation (only for `scheduled` status runs).

### Test Run Detail
Shows use cases in the run, assigned testers, statuses, free pass toggles. Admin and Test Lead can manage. Shows link to Defect Log when run has defects. Delete button for scheduled runs.

### Tester Dashboard (`/tester/dashboard`)
Shows assigned Test Runs for the logged-in tester. Each card shows:
- Project name, run name, scheduled date, countdown timer
- Number of assigned use cases, pending count
- **Status badge**: NEW (blue) / IN PROGRESS (orange) / COMPLETED (green)
- "Start Testing" button (navigates to execution view), "View" button (read-only for completed), "Locked" (not yet available)
...
- Sidebar indicators (checkmarks/X marks) scoped to the current `testRunId` only
...
- **Completed test runs**: shows full-page "Test Run Submitted" message when `testRun.status === "completed"`
- **Existing execution**: if a test case already has a completed execution in this test run, shows read-only results (step-by-step pass/fail with actual results and comments) instead of "Start New Execution"
- Uses `key={`${testRunId}-${testCase.id}`}` to force React remount and invalidates query cache on mount to avoid stale data from previous test runs
...
- Meeting Type: `defect_review` or `post_mortem` (snake_case values; UI displays as "Defect Review" / "Post-Mortem")
...
### Tester Dashboard (`/tester/dashboard`)
Shows assigned Test Runs for the logged-in tester. Each card shows: project name, run name, scheduled date, countdown timer, number of assigned use cases, pending count. Status badge (NEW / IN PROGRESS / COMPLETED). "Start Testing" / "View" / "Locked" button based on run state.

### Test Execution View (`/tester/:projectCode`)
- Shows the Test Link with a "Share to Mobile" button
- Lists assigned Use Cases → Test Cases → Test Steps, scoped to the current test run
- Per Test Case: iteration number (auto-assigned), tester name (pre-filled from login)
- Per Test Step: Actual Result (text + file attachment), Comments (text + file attachment), Pass/Fail toggle
- On completion of a Test Case, set status and trigger sync to update the Test Run use case status
- If a Test Case is failed, the system automatically creates a Defect in the background
- Completed test runs show a full-page "Test Run Submitted" message
- Test cases with existing completed executions show read-only results instead of "Start New Execution"
- Query cache is invalidated on mount to ensure fresh data per test run
...
### Auth Functions
- **`authenticate`**: Verifies JWT from `Authorization: Bearer <token>`. Public paths: `/auth/login`, `/auth/register`, `/health`.
- **`authorize(roles[])`**: Checks `req.user.role` against allowed global roles.
- **`authorizeProjectRole(allowedProjectRoles[])`**: Routes with `:projectId` param. ADMIN bypasses. Looks up `project_assignments` for the user.
- **`checkProjectRole(req, projectId, allowedProjectRoles)`**: Inline helper for routes without `:projectId` (e.g., `PUT /use-cases/:useCaseId`). Returns boolean.
...
12. **Global roles** are `ADMIN` and `USER` only. The `AUTHOR` and `TESTER` global roles do not exist — those are project-level roles only. The registration schema and UI role dropdowns must only show `ADMIN` and `USER`.
13. **`test_run_use_cases.status`** must support `passed_by_agreement` as a valid value in addition to `pending`, `in_progress`, `passed`, `failed`.
14. All API responses must serialize timestamps as ISO-8601 strings.
15. **Raw `fetch()` calls** in frontend code must include `Authorization: Bearer <token>` header via `getAuthToken()`. Do not rely on the generated API client's auth interceptors for direct fetch calls.
16. **`meeting_type`** on team discussions uses snake_case values (`defect_review`, `post_mortem`). Frontend must use snake_case when sending API requests and format for display as needed.
